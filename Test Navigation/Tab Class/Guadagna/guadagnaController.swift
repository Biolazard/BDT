        ...
        dataBase.child("Post").observe(.value) { (snapshot) in
            if let dictionary = snapshot.value as? [String : Any]
            {
                ...
                for element in dictionary
                {
                    ...
                    if post.idBoss == self.userID && post.richiestaofferta == "fornisce" && post.feedbackRilasciatoBoss == false || self.userID == post.proposte && post.richiestaofferta == "richiede" && post.feedbackRilasciatoHelper == false
                    {
                        myPost.append(post)
                    }
